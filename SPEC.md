...
`web`の行末調整が「条件付き」なのは、行末のためだけにJSフォールバックを
起動しないという意味です。`text-spacing-trim: trim-both`が使えるブラウザでは
CSSに任せ、使えないブラウザでは行末調整を見送ります。行末を必ず揃えたい場合は
`book`または`editorial`を選びます。
