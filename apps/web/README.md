...
| `lib/` | URL・メタデータ・フォント、ドキュメントのコード着色 |

ヘッダーはバー（ブランド＋nav）と`header-menu.tsx`のパネル（言語・テーマ）に分けて
います。760px以下ではnavもパネルへ移し、バーを1段のままスクロール追従させます。

文字サイズは`globals.css`のトークンで階層化しています。`--text-3xs`〜`--text-4xl`が
UI（ゴシック）、`--display-1`〜`--display-6`が見出し（明朝）です。見出しはブレイク
ポイントと`:lang()`でトークン自体を再定義するため、個々のセレクタは触りません。
