...
`DocsSection`は`code`を必須とし、コンポーネントも常に`<Code>`を描画します。比較表
やAPI一覧の節を足すには、`code`を任意にし、表とリストを持てるようにする必要が
あります。小さな変更ですが、`content.test.ts`の構造一致テストがあるため日英同時に
行います。
...
増やす作業が、配列へ要素を1つ足すだけになります。コピーボタンはクライアント
コンポーネントとして分離します。
...
みる」（`/playground/`）の2本にします。ヘッダーのナビゲーションにも`/start/`を
加え、`/docs/`より前に置きます。
