...
2. 独自ドメイン（`cdn.mojikumi.jp`）のCDNは初期の必須要件から外します。公開済み
   のnpmパッケージはjsDelivrがそのまま配信できるため、Phase 1はDNSやCDN契約なし
   で完了できます。独自ドメインは後からの別名として追加します。
3. 新パッケージは作らず、既存の`mojikumi`パッケージへブラウザ向け成果物を追加し
...
```html
<script
  src="https://cdn.jsdelivr.net/npm/mojikumi@0.1.2/dist/mojikumi.browser.js"
...
`content.test.ts`が日英の構造一致を検査するため、新しいページは日英同時に追加
する必要があります。英語版を後回しにしたい場合は、先にテストと型の扱いを決める
必要があります（未決事項を参照）。
...
- `cdn.mojikumi.jp`をjsDelivrの別名として追加し、既存URLは維持します。
...
1. **英語版の同時提供**：新規ページは日英の構造一致がテストで強制されます。18
   ページ分を両言語で用意するか、日本語先行を許す仕組みへ変えるか。
2. **CDNのURL**：初期からjsDelivrの固定バージョンURLで進める方針でよいか。
   （`https://cdn.jsdelivr.net/npm/mojikumi@0.1.2/dist/mojikumi.browser.js`）
3. **バージョン固定**：初心者向けに固定バージョンを案内するか、`@0.1`のような
   範囲指定にするか。固定は安全ですが、更新のたびに貼り直しが必要です。

## 最初の実装単位

Phase 1（ブラウザバンドルと自動初期化）を単独のPull Requestにします。Phase 2以降
はすべてこの成果物の上に乗るため、ここで属性名とセレクターの既定値を確定させる
ことが、後続の手戻りを防ぎます。
